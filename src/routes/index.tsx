...
function Home() {
  ...
  const slogans = language === "sl"
    ? [
        "Mi načrtujemo. Vi potujete.",
        "Manj načrtovanja. Več raziskovanja.",
        "Vi sanjate. Mi načrtujemo.",
      ]
    : [
        "We Plan. You Travel.",
  ...
  useEffect(() => {
    const id = setInterval(() => setSloganIndex((s) => (s + 1) % slogans.length), 3500);
    return () => clearInterval(id);
  }, [slogans.length]);

  const reasons = language === "sl"
    ? [
        { icon: Sparkles, title: "Brez stresa", desc: "Poskrbimo za vsako podrobnost." },
        { icon: Clock, title: "Prihranite ure raziskovanja", desc: "Zamudno iskanje prepustite nam." },
        { icon: Tag, title: "Pregledne cene", desc: "Jasni stroški organizacije, brez presenečenj." },
        { icon: Wand2, title: "Potovanja po meri", desc: "Oblikovana povsem po vaših željah." },
        { icon: Ban, title: "Brez skritih stroškov", desc: "Vedno točno veste, kaj plačate." },
        { icon: FileCheck, title: "Pomoč pri vizumih", desc: "Pregled pogojev in enostavni postopki." },
        { icon: Plane, title: "Načrtovanje letov", desc: "Najboljše povezave in optimalni časi." },
        { icon: BedDouble, title: "Izbira nastanitev", desc: "Skrbno izbrani in preverjeni hoteli ter vile." },
        { icon: LifeBuoy, title: "Podpora na poti", desc: "Dosegljivi smo za vas med samim potovanjem." },
      ]
    : [
        ...
        { icon: LifeBuoy, title: "Support Available", desc: "We are here while you travel." },
      ];

  const steps = language === "sl"
    ? [
        { icon: ClipboardCheck, title: "Izpolnite vprašalnik", desc: "Zaupajte nam svoje želje v nekaj minutah." },
        { icon: Search, title: "Pregled vaših želja", desc: "Podrobno preučimo vaš vprašalnik in proračun." },
        { icon: CreditCard, title: "Potrditev načrtovanja", desc: "Varno plačilo storitve za pričetek priprave." },
        { icon: MapPinned, title: "Prejem ponudbe", desc: "Prilagojen, natančen in pripravljen potovalni načrt." },
        { icon: CalendarCheck, title: "Ureditev rezervacij", desc: "Leti, nastanitve in aktivnosti so potrjeni." },
        { icon: PlaneTakeoff, title: "Uživajte na poti", desc: "Preprosto potujte, za ostalo poskrbimo mi." },
      ]
    : [
        ...
        { icon: CalendarCheck, title: "Bookings are made", desc: "Flights, stays, and activities are confirmed." },
  ...
  const testimonials = language === "sl"
    ? [
        { name: "Sofia & Marc", trip: "Maldivi · 10 dni", quote: "Zdelo se je, kot da nam berejo misli. Vsaka podrobnost je bila popolna, od resorta do transferja s hidroplanom." },
        { name: "Amelia R.", trip: "Japonska · 3 tedni", quote: "Prihranila sem ogromno ur iskanja. Načrt poti je tekel čudovito, brez kakršnegakoli stresa pri organizaciji." },
        { name: "David L.", trip: "Bali · Družina", quote: "Potovanje z otroki je lahko naporno, vendar je bilo z Viarro povsem preprosto. Vredno vsakega evra." },
      ]
    : [
        ...
        { name: "Amelia R.", trip: "Japan · 3 weeks", quote: "I saved so many hours. The itinerary flowed beautifully, with no travel planning stress at all." },
        { name: "David L.", trip: "Bali · Family", quote: "Traveling with kids can be chaos, but Viarra made it feel effortless. Worth every euro." },
      ];
    ...
    <div>
        ...
        <div className="relative mx-auto flex min-h-screen max-w-7xl flex-col justify-center px-6 pt-32 pb-20">
          ...
          >
            ...
            <h1 className="mt-6 font-display text-6xl md:text-8xl font-bold leading-[0.95] tracking-tight text-white drop-shadow-lg">
              {language === "sl" ? (
                <>Načrtujte manj.<br />Potujte več.</>
              ) : (
                <>Plan Less.<br />Travel More.</>
              )}
            </h1>

            <p className="mt-6 max-w-xl text-lg text-white/95 drop-shadow leading-relaxed">
              {language === "sl"
                ? "Ustvarjamo povsem prilagojena potovalna doživetja po vaši meri. Letalske karte, nastanitve, vizumi, prevozi, poti in aktivnosti, vse organizirano prek spleta."
                : "We create fully personalised travel experiences tailored around you. Flights, accommodation, visas, transport, routes, and activities, all organised online."}
            </p>

            <div className="mt-9 flex flex-wrap gap-3">
              <a
                href="#quick-planner"
                className="group inline-flex items-center gap-2 rounded-full bg-primary px-7 py-4 text-sm font-semibold text-primary-foreground shadow-soft transition-all hover:bg-accent hover:shadow-xl cursor-pointer"
              >
                {t("startPlanning")}
              ...
              >
                {language === "sl" ? "Več o nas" : "Learn More"}
              </Link>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="font-display text-4xl md:text-5xl font-bold">
              {language === "sl" ? "Zakaj izbrati Viarro" : "Why choose Viarra"}
            </h2>
            <p className="mt-4 text-muted-foreground">
              {language === "sl"
                ? "Sodobna storitev načrtovanja, zgrajena na zaupanju, preglednosti in izkušnjah."
                : "A modern planning service built on trust, transparency and taste."}
            </p>
          </div>
      ...
      <section className="px-6 py-24 gradient-soft">
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-2xl text-center">
            <div className="inline-flex items-center gap-2 rounded-full glass px-4 py-1.5 text-xs font-medium">
              <Shield size={14} /> {language === "sl" ? "Kako deluje" : "How it works"}
            </div>
            <h2 className="mt-4 font-display text-4xl md:text-5xl font-bold">
              {language === "sl" ? "Šest korakov do vašega potovanja" : "Six steps to your trip"}
            </h2>
          </div>
      ...
      <section className="px-6 py-24">
        <div className="mx-auto max-w-7xl">
          <div className="flex items-end justify-between gap-6 flex-wrap">
            <div>
              <h2 className="font-display text-4xl md:text-5xl font-bold">
                {language === "sl" ? "Priljubljene destinacije" : "Featured destinations"}
              </h2>
              <p className="mt-3 text-muted-foreground">
                {language === "sl" ? "Nekaj krajev, ki jih z veseljem načrtujemo." : "A few places we love to plan."}
              </p>
            </div>
            <Link to="/gallery" className="text-sm font-medium text-foreground/70 hover:text-foreground">
              {language === "sl" ? "Oglejte si galerijo →" : "See gallery →"}
            </Link>
          </div>
      ...
      <section className="px-6 py-24 gradient-soft">
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="font-display text-4xl md:text-5xl font-bold">
              {language === "sl" ? "Potniki, ki nam zaupajo" : "Travellers who trust us"}
            </h2>
          </div>
      ...
      <section className="px-6 py-28">
        <div className="mx-auto max-w-5xl overflow-hidden rounded-[2.5rem] gradient-beach p-12 md:p-16 shadow-soft text-center">
          <h2 className="font-display text-4xl md:text-6xl font-bold">
            {language === "sl" ? "Ste pripravljeni na svoje potovanje?" : "Ready to start planning?"}
          </h2>
          <p className="mt-4 text-foreground/80 max-w-xl mx-auto">
            {language === "sl"
              ? "Vaše sanjsko potovanje je bližje, kot si mislite. Oblikujmo ga skupaj."
              : "Your dream trip is closer than you think. Let's design it together."}
          </p>
          <Link
            to="/planner"
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-foreground px-8 py-4 text-sm font-semibold text-background hover:bg-foreground/90 transition-all cursor-pointer"
          >
            {language === "sl" ? "Začnite svoje potovanje" : "Start Your Journey"} <ArrowRight size={16} />
          </Link>
        </div>
      </section>
    </div>
...
function Pricing({ language }: { language: string }) {
  const plans = language === "sl"
    ? [
        { name: "Standardno načrtovanje", price: "150 €", desc: "Do 2 odrasli osebi", highlight: true },
        { name: "Dodatna odrasla oseba", price: "30 €", desc: "Na vsako dodatno odraslo osebo" },
        { name: "Dodaten otrok", price: "15 €", desc: "Na otroka" },
        { name: "Krajši oddih (City Break)", price: "80 €", desc: "Načrtovanje krajšega oddiha do 5 dni" },
      ]
    : [
  ...
  const support = language === "sl"
    ? [
        { days: "5 dni", price: "80 €" },
        { days: "14 dni", price: "100 €" },
        { days: "30 dni", price: "150 €" },
      ]
    : [
  ...
  return (
    <section className="px-6 py-24">
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="font-display text-4xl md:text-5xl font-bold">
            {language === "sl" ? "Preproste in pregledne cene" : "Simple, transparent pricing"}
          </h2>
          <p className="mt-4 text-muted-foreground">
            {language === "sl"
              ? "Plačilo storitve krije načrtovanje in organizacijo. Letalske karte, nastanitve, prevozi in vizumske takse se plačajo neposredno ponudnikom storitev."
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2">
          <div className="rounded-3xl bg-card p-8 shadow-soft">
            <h3 className="font-display text-xl font-semibold">
              {language === "sl" ? "Paketi podpore na poti" : "Support Packages"}
            </h3>
          ...
          <div className="rounded-3xl bg-card p-8 shadow-soft flex flex-col justify-between">
            <div>
              <h3 className="font-display text-xl font-semibold">
                {language === "sl" ? "Pomoč pri vizumih" : "Visa Assistance"}
              </h3>
              <p className="mt-2 text-sm text-muted-foreground">
                {language === "sl"
                  ? "Celostno vodenje postopka in pregled potrebne dokumentacije."
                  : "Full paperwork guidance and documentation review."}
              </p>
            </div>
            <div className="mt-6 font-display text-4xl font-bold">
              {language === "sl" ? "25 € do 45 €" : "€25 to €45"}
            </div>
          </div>
        </div>
        ...
        <p className="mt-8 text-center text-xs text-muted-foreground max-w-2xl mx-auto">
          {language === "sl"
            ? "Pomembno: Plačilo storitve krije načrtovanje. Letalske karte, nastanitve, prevozi in vizumi se plačajo neposredno ponudnikom storitev."
